"""
Script to launch Halmos within the benchmark framework and format the results for the Confusion Matrix.
"""
from pathlib import Path
import argparse
import subprocess
import csv
import sys
import re
import utils

DEFAULT_TIMEOUT = '10m'

def parse_timeout_to_seconds(timeout_str):
    try:
        if timeout_str.endswith('m'):
            return int(timeout_str[:-1]) * 60
        elif timeout_str.endswith('s'):
            return int(timeout_str[:-1])
        return int(timeout_str)
    except ValueError:
        return 600

##
def check_test_exists_in_contracts(halmos_dir, clean_p):
    """
    Scansiona i file .t.sol (o .sol) nella directory di Halmos per verificare
    se esiste una funzione di test che contiene 'clean_p'.
    """
    halmos_path = Path(halmos_dir)
    if not halmos_path.exists():
        return True  # Fallback di sicurezza: se la cartella non esiste, lascia eseguire Halmos
        
    # Cerca in tutti i file .t.sol o .sol dentro la cartella halmos
    for sol_file in halmos_path.rglob("*.sol"):
        try:
            content = sol_file.read_text(encoding="utf-8")
            # Cerca la definizione di una funzione che contiene clean_p (es. function check_...)
            pattern = rf"function\s+[a-zA-Z0-9_]*{re.escape(clean_p)}[a-zA-Z0-9_]*\s*\("
            if re.search(pattern, content, re.IGNORECASE):
                return True
        except Exception:
            continue
            
    return False


def extract_test_contract_name(file_path, clean_p):
    """
    Estrae il nome del contratto che racchiude la funzione di test 'clean_p'.
    Se non lo trova con precisione, restituisce il nome dell'ultimo contratto definito nel file.
    """
    try:
        content = Path(file_path).read_text(encoding="utf-8")
        
        test_search = clean_p
        if test_search.startswith("check_"):
            test_search = test_search[6:]
        elif test_search.startswith("invariant_"):
            test_search = test_search[10:]

        # Cerca tutti i contratti definiti nel file
        contracts = re.findall(r'\bcontract\s+([a-zA-Z0-9_]+)', content)
        if not contracts:
            return None

        # Se c'è solo un contratto, restituiamo quello
        if len(contracts) == 1:
            return contracts[0]

        # Se ce ne sono multipli (es. Sink e BankTest), cerchiamo quello che contiene il test
        # Splittiamo il file per la parola chiave 'contract '
        parts = content.split("contract ")
        for part in parts[1:]:
            lines = part.splitlines()
            c_name = lines[0].split()[0].split('{')[0].strip()
            if test_search.lower() in part.lower():
                return c_name

        # Fallback: restituisce l'ultimo contratto definito nel file (solitamente il Test Contract principale)
        return contracts[-1]

    except Exception:
        return None

def run_halmos_for_task(p, v, halmos_dir, output_dir, timeout_seconds):
    clean_p = p.replace('-', '_')
    halmos_path = Path(halmos_dir)
    target_file_path = None

    # 1. Trova il file fisico .t.sol (con o senza prefisso di versione)
    if v:
        matches = list(halmos_path.rglob(f"{v}_{p}.t.sol")) + list(halmos_path.rglob(f"{v}_{clean_p}.t.sol"))
        if matches:
            target_file_path = matches[0]

    if not target_file_path:
        matches = list(halmos_path.rglob(f"{p}.t.sol")) + list(halmos_path.rglob(f"{clean_p}.t.sol"))
        if matches:
            target_file_path = matches[0]

    # 2. CONTROLLO DI ESISTENZA RAPIDO
    if not check_test_exists_in_contracts(halmos_dir, clean_p):
        return utils.UNKNOWN

    print(f"Running Halmos verification for property: '{p}', version: ({v}) -> File: {target_file_path}")

    try:
        # --- ESTRAZIONE NOME PROPRIETÀ/TEST ---
        test_match = clean_p
        if test_match.startswith("check_"):
            test_match = test_match[6:]
        elif test_match.startswith("invariant_"):
            test_match = test_match[10:]

        # --- COSTRUZIONE COMANDO HALMOS ---
        halmos_cmd = [
            "halmos",
            "--match-test", test_match,
            "--solver-timeout-assertion", "120000"
        ]

        # --- DEDUZIONE DINAMICA E ACCURATA DEL CONTRATTO DI TEST ---
        if target_file_path and target_file_path.exists():
            contract_name = extract_test_contract_name(target_file_path, clean_p)
            if contract_name:
                # Isola l'esecuzione SOLO sul contratto che definisce realmente il test
                halmos_cmd.extend(["--contract", contract_name])

        # Esecuzione processo Halmos
        halmos_res = subprocess.run(
            halmos_cmd,
            cwd=halmos_dir, 
            capture_output=True, 
            text=True,
            timeout=timeout_seconds
        )
        
        output = halmos_res.stdout + "\n" + halmos_res.stderr
        
        # Salvataggio Log
        logs_dir = Path(output_dir).joinpath("logs")
        logs_dir.mkdir(parents=True, exist_ok=True)
        log_filename = f"{v}_{p}.log" if v else f"{p}.log"
        utils.write_log(logs_dir.joinpath(log_filename), output)
        
        # Rimozione colori/codici ANSI dall'output
        ansi_escape = re.compile(r'\x1B(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])')
        clean_output = ansi_escape.sub('', output)
        
        # --- PARSING DEI RISULTATI ---
        fail_pattern = rf"\[FAIL\]\s+.*{re.escape(test_match)}"
        pass_pattern = rf"\[PASS\]\s+.*{re.escape(test_match)}"
        
        # 1. Priorità al FAIL
        if re.search(fail_pattern, clean_output, re.IGNORECASE) or (
            re.search(rf"{re.escape(test_match)}", clean_output, re.IGNORECASE) and "Counterexample:" in clean_output
        ):
            return utils.STRONG_NEGATIVE  # Maps to 'N!'

        # 2. Se non c'è FAIL, verifichiamo il PASS
        elif re.search(pass_pattern, clean_output, re.IGNORECASE):
            return utils.STRONG_POSITIVE  # Maps to 'P!'

        # 3. Fallback linea per linea
        else:
            lines = [line for line in clean_output.split('\n') if test_match.lower() in line.lower()]
            for line in lines:
                if "[fail]" in line.lower():
                    return utils.STRONG_NEGATIVE
                if "[pass]" in line.lower():
                    return utils.STRONG_POSITIVE
            
            return utils.UNKNOWN

    except subprocess.TimeoutExpired:
        return utils.TIMEOUT
    except Exception as e:
        print(f"Error during Halmos execution for {p}: {e}")
        return utils.ERROR

def main(args_list=None):
    parser = argparse.ArgumentParser()
    parser.add_argument('--halmos-dir', '-hd', help='Halmos working directory.', required=False)
    parser.add_argument('--contracts', '-c', help='Contracts file or directory.', required=True)
    parser.add_argument('--output', '-o', help='Output directory.', required=True)
    parser.add_argument('--timeout', '-t', help='Timeout time.', required=False)
    parser.add_argument('--version', '-v', help='Run on this version only.', required=False)
    parser.add_argument('--property', '-p', help='Run on this property only.', required=False)
    
    if args_list is not None:
        args = parser.parse_args(args_list)
    else:
        args = parser.parse_args()

    # ---- HALMOS_DIR ----
    if args.halmos_dir:
        halmos_dir = Path(args.halmos_dir)
    else:
        halmos_dir = Path("./halmos")

    output_dir = Path(args.output)
    output_dir.mkdir(parents=True, exist_ok=True)
    
    timeout = args.timeout if args.timeout else DEFAULT_TIMEOUT
    timeout_seconds = parse_timeout_to_seconds(timeout)
    
    tasks = []
    
    # 1. Ricerca del ground-truth.csv
    gt_path = Path("../ground-truth.csv")
    if not gt_path.exists():
        gt_path = Path("./ground-truth.csv")

    # 2. Popolamento dei task in base alle flag ricevute
    if args.property and args.version:
        # Caso specifico: una singola proprietà su una singola versione
        tasks.append((args.property, args.version))

    elif args.property and not args.version:
        # Caso in cui è specificata solo la proprietà: cerchiamo tutte le versioni per quella proprietà nel CSV
        if gt_path.exists():
            with open(gt_path, 'r') as f:
                reader = csv.reader(f)
                next(reader)
                for row in reader:
                    if row and len(row) >= 2 and row[0] == args.property:
                        tasks.append((row[0], row[1]))
        if not tasks:
            # Fallback se non trovata nel CSV
            tasks.append((args.property, 'v1'))

    else:
        # Caso generale: caricamento dal CSV con eventuale filtro su --version
        if gt_path.exists():
            with open(gt_path, 'r') as f:
                reader = csv.reader(f)
                next(reader)
                for row in reader:
                    if row and len(row) >= 2:
                        prop_name, ver_name = row[0], row[1]
                        # Se l'utente ha passato --version, filtriamo solo quella versione!
                        if args.version and ver_name != args.version:
                            continue
                        tasks.append((prop_name, ver_name))
        else:
            tasks.append(("unknown-property", "v1"))
            
    current_results = {}
    for p, v in tasks:
        res = run_halmos_for_task(p, v, halmos_dir, output_dir, timeout_seconds)
        current_results[(p, v)] = res

    # Salvataggio e aggiornamento di out.csv
    out_csv_path = output_dir.joinpath('out.csv')
    existing_rows = []
    if out_csv_path.exists():
        try:
            with open(out_csv_path, 'r') as f:
                reader = csv.reader(f)
                next(reader)
                for row in reader:
                    if row:
                        if (row[0], row[1]) not in current_results:
                            existing_rows.append(row)
        except Exception:
            pass

    out_csv = [utils.OUT_HEADER] + existing_rows
    for (p, v), res in current_results.items():
        out_csv.append([p, v, res])

    with open(out_csv_path, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerows(out_csv)
        
    for (p, v), res in current_results.items():
        print(f"Halmos result appended for {p} ({v}): {res}")

if __name__ == '__main__':
    main()